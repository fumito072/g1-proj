...
| 停止後の後退への対策(9/4 夕) | `real/autowalk.py`(`_final_approach`, `_log_post_stop`=アンカー保持) | 後ろ向きの指令は出ていないのに停止直後に 20〜50 cm 後退。減速を早めて実速度を落としてから止め、止めた後 6 秒は位置を見張って戻す。停止距離の内側では後ろへ歩かない。docs/自動歩行 §6b-11 |
| 目的地でロック立位(9/4 夕) | `real/autowalk.py`(`_lock_stand`, `_unlock_if_needed`) | 止まったら足踏みをやめて FSM 4 で静止。次の前進で自動で歩行へ戻る。回り込み不成立の理由をログに。docs/自動歩行 §6b-12 |
| 止め際の足踏み対策(9/4 夕) | `real/autowalk.py`(`_wait_still`, 実速度で減速) | 速いまま止めていた(最後の区間で実速度 0.4 m/s)。実速度で早めに緩く減速、実速度 0.15 以下で最後の区間へ、静止 1 秒を待ってからロック立位、FSM を読んで確認。docs/自動歩行 §6b-13 |
| 根本の見直し(9/4 夕) | `real/real_robot.py`(`enter_standing_loco`)、`real/autowalk.py` | FSM 4 は 200 から入れない(実測)→ 目的地では 802(速度指令を受けない立位)で静止。壁の最終接近と静止判定は LiDAR を真に(オドメトリは足踏みを数える)。歩行モードは 200 を先に(6 秒の無駄を除去)。docs/自動歩行 §6b-14 |
| いちからの設計(9/4 夜、未検証) | `real/autowalk.py` 全面書き直し、`real/real_robot.py`、`docs/歩行SDK調査_20260904.md` | 調査(公式 FSM 表・SDK 履歴・issue・実装例)に基づき、歩行=501(だめなら200)+SetBalanceMode(0)、指令=LiDAR 残り距離に比例(下限 0.30)、停止=ゼロ→fsm_mode==0 を待つ、FSM は切り替えない。802 は走跑モードで静止用に使わない。docs/自動歩行 §6b-15 |
| 書き直しのレビューと修正(9/4 夜) | `real/autowalk.py`、`real/real_robot.py`、`real/test_autowalk.py` | 敵対的レビュー(3観点×2懐疑者)で確定した 16 点を修正。指令が下限未満の帯を通らない、回り込み中の別障害物を見る、停止判定は LiDAR のコマ単位、RPC 待ち 0.25秒、ゼロ指令の持続、静止待ちの厳格化ほか。docs/自動歩行 §6b-16 |
